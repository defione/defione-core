...
// will be transferred to a governance smart contract once Defione is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accDefionePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDefionePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accDefionePerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }

    // The Defione TOKEN!
    DefioneToken public Defione;
    // Dev address.
    ...
    // Block number when bonus Defione period ends.
    uint256 public bonusEndBlock;
    // Defione tokens created per block.
    uint256 public DefionePerBlock;
    //Amount of sushis per Second
    uint256 public DefionePerSecond;
    // Bonus muliplier for early Defione makers.
    ...
    // The block number when Defione mining starts.
    ...
    event RewardChange(uint256 DefionePerBlock, uint256 Blocktime);
   ...
   modifier validPID(uint256 _pid){
    ...
    constructor(
        ...
        DefionePerBlock = _sushiPerBlock;
        DefionePerSecond = _sushiPerSecond;
    ...
    function updateBlockIssuanceWithBlocktime()
        public onlyOwner {
        require(lastBlocktime != currentBlocktime , "Defione : Blocktime hasn't changed");
        DefionePerBlock = currentBlocktime.mul(DefionePerSecond).div(1000);
        lastBlocktime = currentBlocktime;
        emit RewardChange(DefionePerBlock, currentBlocktime);
    }

    //Changes the Blocktime Parameter and updates the Rewards per Block
    function changeBlocktime(uint _currentBlocktime)
        public onlyOwner {
        require(lastBlocktime != _currentBlocktime , "Defione : Blocktime hasn't changed");
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accDefionePerShare: 1
            })
        );
    }

    // Update the given pool's Defione allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function pendingDefione(uint256 _pid, address _user)
        external
        view
        validPID(_pid)
        returns (uint256)
        ...
        uint256 accDefionePerShare = pool.accDefionePerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 DefioneReward =
                multiplier.mul(DefionePerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accDefionePerShare = accDefionePerShare.add(
                DefioneReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accDefionePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public validPID(_pid) {
        ...
        Defione.mint(devaddr, DefioneReward.div(devDivisor));
        Defione.mint(address(this), DefioneReward);
        pool.accDefionePerShare = pool.accDefionePerShare.add(
            DefioneReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Defione allocation.
    function deposit(uint256 _pid, uint256 _amount) public validPID(_pid) nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDefionePerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDefionePerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validPID(_pid) nonReentrant{
        ...
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accDefionePerShare).div(1e12).sub(
                user.rewardDebt
            );
        safeTokenTransfer(msg.sender, pending);
    ...
    //Withdraw users Defione token by Collector address. Allows some flexibility to the dev.
    //Does not withdraw LPs
    function extWithdraw(uint256 _pid, address _user)
    public onlyCollector validPID(_pid) nonReentrant{
        uint256 _amount = 0; //HardCoded 0 Amount: Withdraw only Defione tokens and not LP tokens
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
    ...
    // Safe Defione transfer function, just in case if rounding error causes pool to not have enough SUSHIs.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = Defione.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBal) {
            transferSuccess = Defione.transfer(_to, tokenBal);
        } else {
            transferSuccess = Defione.transfer(_to, _amount);
        }
